...
将 WHU Building Dataset 中的 512×512 遥感图像放大至 **2048×2048**（特征网格 128×128），用 DINOv3 第 19 层特征计算每个 patch 的范数，取前 1% 最强响应做连通域分析，共可检测出 **95 个候选区域**。

| 项目 | 要求 |
|------|------|
| GPU | 4GB+ VRAM（实测 2048×2048 仅需 **2.09GB**） |
| 内存 | 8GB+ |
| Python | 3.10+ |
| CUDA | 11.8+ |
...
|------|-----|
| 输入尺寸 | 2048×2048 |
| 特征网格 | 128×128（DINOv3 patch=16） |
| 使用层 | Layer 19（深层语义） |
| 阈值 | 前 1% 分位数 |
| 显存占用 | **2.09 GB**（GTX 1060 6GB） |
| 检测候选区 | **95 个**（全部为小目标候选） |

95 个候选区均为"小目标"（面积 < 画面 0.5%），包含车辆、小建筑等。部分候选区可能为相邻目标的重复响应（因放大插值后同一目标覆盖多个 patch），实际独立目标数低于 95。
...
- **需要合理阈值**：前 1% 分位数在本图上效果最佳，换图可能需要微调
