# 《视觉自监督模型DINOv3：原理、训练到部署》

- ### 本仓库为本书的勘误更新和扩展阅读页

- ### 本书完整源码、数据和配套资源请见本书前言中的二维码资源包

![图书](https://5starsoft.com.cn/dinov3.jpg)

### 本书由清华大学出版社出版     [图书出版社链接](https://www.tup.tsinghua.edu.cn/booksCenter/book_11572701.html)

---

## 目录

### 第一部分：基础篇

- [第一章 DINOv3全景导览与学习指南](./chapter01/README.md)
- [第二章 DINOv3的训练原理与核心机制](./chapter02/README.md)
  - 🧪 [扩展：DINOv3 ViT-Large 24层语义层级参考](./chapter02/README.md#-扩展实验dinov3-vit-large-24层语义层级参考)
- [第三章 DINOv3实验配置要求与环境搭建](./chapter03/README.md)

### 第二部分：应用篇

- [第四章 特征提取：视觉基础表示](./chapter04/README.md)
  - 🧪 [扩展：特征提取在医学影像上的应用](./chapter04/README.md#-扩展实验特征提取在医学影像上的应用)
- [第五章 零样本分类：文本与图像对齐](./chapter05/README.md)
  - 🧪 [扩展：DINOv3与词向量零样本分类](./chapter05/README.md#-扩展实验dinov3与词向量零样本分类)
- [第六章 语义分割：像素级分类](./chapter06/README.md)
  - 🧪 [扩展：DINOv3 皮肤病变语义分割](./chapter06/README.md#-扩展实验dinov3-皮肤病变语义分割)
- [第七章 目标检测：定位与识别](./chapter07/README.md)
  - 🧪 [扩展：DINOv3 零训练特征遥感小目标检测](./chapter07/README.md#-扩展实验dinov3-遥感小目标检测零训练特征可视化)
- [第八章 特征匹配：密集稀疏关联](./chapter08/README.md)
- [第九章 3D赋能：SAM 3D Body应用](./chapter09/README.md)
  - 🧪 [扩展：SAM3 LoRA 微调 ISIC 2018 病灶分割](./chapter09/README.md#-扩展实验sam3-lora-微调isic-2018-病灶分割)

### 第三部分：训练篇

- [第十章 DINOv3图像分类：轻量级任务头训练](./chapter10/README.md)
- [第十一章 DINOv3图像分类：基于自有数据训练](./chapter11/README.md)
- [第十二章 DINOv3语义分割：卫星遥感图像分割训练](./chapter12/README.md)
  - 🧪 [扩展：DINOv3.sat493m遥感建筑分割训练](./chapter12/README.md#-扩展实验whu-遥感建筑分割dinov3--dpt-head)
- [第十三章 DINOv3目标检测：基于COCO数据集训练](./chapter13/README.md)
- [第十四章 DINOv3知识蒸馏：构建轻量化视觉模型](./chapter14/README.md)
- [第十五章 DINOv3蒸馏模型：轻量化图像分割](./chapter15/README.md)
  - 🧪 [扩展：EUPE-ViT-B 遥感线性分割和特征融合分割](./chapter15/README.md#-扩展实验eupe-vit-b-遥感线性分割)
- [第十六章 DINOv3扩展应用：YOLO和DEIMv2应用](./chapter16/README.md)

---

> - ### 🧪 扩展实验为图书之外的独立补充内容，建议在掌握对应章节基础后学习。
